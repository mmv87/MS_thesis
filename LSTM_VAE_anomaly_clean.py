...
## the data is downsampled for sake of computation
...
## parameters based on input dimension of 'X'
...
hist=elbo_vae.fit({'enc_inp_1':x_train,'enc_inp_2':x_train},epochs=350,batch_size=64)
...
plt.show()
##to save the model
save_file_name="D:/upgrad_course/LJMU_Masters_thesis/paper_repo/dataset/WADI_dataset/LSTM_vae_Robot/LSTM_vae_model_saved/res/LSTM_vae_2_custom_layers_lat_dim_2.keras"
elbo_vae.save(save_file_name)

##load the model
##custom_object={'CustomModel':VAE}
elbo_vae_loaded=tf.keras.saving.load_model('D:/upgrad_course/LJMU_Masters_thesis/paper_repo/dataset/WADI_dataset/LSTM_vae_Robot/LSTM_vae_model_saved/res/LSTM_vae_2_custom_layers_lat_dim_2.keras')                               
##                                      
# %%
#%%
...
##df_train=pd.DataFrame(dict_z_train)
...
for i in range(2):
    print('mu =' ,dense_val[i][:9])
    print('sigma=', tf.math.softplus(dense_val[i][9:]))

for i in range(5):
    print('mu =' ,dense_train[i][:9])
    print('sigma=', tf.math.softplus(dense_train[i][9:]))

for i in range(2):
    print('mu =' ,dense_test[i][:9])
    print('sigma=', tf.math.softplus(dense_test[i][9:]))

##t-sne plot for latent space of dimensions '4'
...
##t-sne plot for latent space of dimensions '6'
from sklearn.manifold import TSNE
...
# %%
#%%
# to calculate the anomaly score  based on threshold (reconstruction_prob)
from scipy.stats import multivariate_normal
def recon_prob(x,dense_out):
    mu_x=dense_out[:,:9]
    sigma_x=dense_out[:,9:]
    batch_size=x.shape[0]
    t_s=x.shape[1]
    f_s=x.shape[2]
    recon_p_window=np.empty((batch_size,t_s,f_s))
    for w in range(batch_size):
        recon_prob=np.empty((t_s,f_s))
        for f in range(f_s):
            p_m=-(multivariate_normal.logpdf(x[w,:,f],mean=mu_x[w,f],cov=np.diag(tf.math.softplus(sigma_x[w,f]))))
            recon_prob[:,f]=p_m
        recon_p_window[w,:,:]=recon_prob
    return recon_p_window

def recon_prob_trial_2(x,dense_out):
    mu_x=dense_out[:,:9]
    sigma_x=dense_out[:,9:]
    batch_size=x.shape[0]
    t_s=x.shape[1]
    ...
    for w in range(batch_size):
        recon_prob=np.empty((t_s,f_s))
        for f in range(f_s):
            log_prob=-(multivariate_normal.logpdf(np.reshape(x[w,:,f],(t_s,1)),
                                               mean=mu_x[w,:],cov=np.diag(tf.math.softplus(sigma_x[w,:]))))
            recon_prob[:,f]=log_prob
        rec_p_window[w,:,:]=recon_prob
    return rec_p_window

##to flatten the recon_prob vs time for validation-data
recon_p_val=recon_prob_trial_2(x_val,dense_val)
recon_p_vae_val=recon_p_val[0,:,:]
for i in range(recon_p_val.shape[0]-1):
    recon_p_vae_val=np.concatenate((recon_p_vae_val,recon_p_val[i+1,-1,:].reshape(1,recon_p_val.shape[2])),axis=0)

## to flatten the recon_prob vs time for the test-data
recon_p_test=recon_prob_trial_2(x_test,dense_test)
recon_p_vae_test=recon_p_test[0,:,:]
for i in range(recon_p_test.shape[0]-1):
    recon_p_vae_test=np.concatenate((recon_p_vae_test,recon_p_test[i+1,-1,:].reshape(1,recon_p_test.shape[2])),axis=0)

recon_p_vae_val.shape
recon_p_vae_test.shape
## to calculate the moving average
def moving_average(x, w):
    return np.convolve(x, np.ones(w), 'valid')/w

##val_window_avg_f1=moving_average(recon_p_vae_val[:,4],windows_size)
## function to calculate moving average for entire feature set
def rec_p_moving_avg(test_data):
    res_test_rp=np.empty([3401,9])
    for i in range(9):
        test_window_avg=moving_average(test_data[:,i],windows_size)
        res_test_rp[:,i]=np.vstack(test_window_avg.reshape(1,3401))
    return res_test_rp

val_rp_averaged_1=rec_p_moving_avg(recon_p_vae_val)
test_rp_averaged_1=rec_p_moving_avg(recon_p_vae_test)
...
##to plot the validated and test data for window averages recon_prob values
fig,ax=plt.subplots(9,1,figsize=(20,15))
for i in range(9):
        ax[i].plot(val_rp_averaged[:3600,i],'b-',label='val_data',linewidth=1.0)
        ax[i].plot(test_rp_averaged[:3600,i],'r-',label='test_data',linewidth=1.0)
        ax[i].tick_params('x',labelrotation=0,labelsize=10)
plt.show()
...
## reconst_prob for entire observation of timeseries
fig,ax=plt.subplots(9,1,figsize=(50,25))
for i in range(9):
    ax[i].plot(recon_p_vae_test[:2600,i],'r-',linewidth=1.0)
    ax[i].plot(recon_p_vae_val[:2600,i],'b-',linewidth=1.0)
plt.show()

# to calculate the anomaly score for the window - average of timeslices in a window
def recon_prob_avg_1(x,dense_out):
    mu_x=dense_out[:,:9]
    sigma_x=dense_out[:,9:]
    batch_size=x.shape[0]
    t_s=x.shape[1]
    f_s=x.shape[2]
    recon_p_window=np.empty((batch_size,f_s))
    for w in range(batch_size):
        recon_prob=np.empty((batch_size,f_s))
        for f in range(f_s):
            rec_p=-(multivariate_normal.logpdf(np.reshape(x[w,:,f],(t_s,1)),
                                               mean=mu_x[w,:],cov=np.diag(tf.math.softplus(sigma_x[w,:]))))
            recon_prob[w,f]=np.mean(rec_p,axis=0)
      ##  recon_p_window[w,:]=recon_prob
    return recon_prob

rec_window_test_1=recon_prob_avg_1(x_test,dense_test)
rec_window_test_1
plt.plot(rec_window_test_1[:,0])

## calculate threshold based on x_val 
def threshold(rec_prob_f):
...
def prep_anomaly(rec_prob_f,threshold):
...
def pred_label(test_data,val_data):
    df_res=pd.DataFrame(columns=['M_1','M_2','M_3','M_4','M_5','M_6','M_7','M_8','M_9'])
    th=threshold(val_data)
    
    for i in range(9):
        df_test_res=prep_anomaly(test_data[:,i],th[i])
        df_res.iloc[:,i]=pd.concat([df_test_res],axis=1)
    return df_res

df_test_full=pred_label(test_rp_averaged_1,val_rp_averaged_1)
